...
using AE.O365.GetFiles.Common.Extensions;

namespace AE.O365.GetFiles.Fromework.CSApp
{
    internal class Program
    {
		static CamlQuery CreateAllFilesQuery()
		{
		...
		static string GetText(string strMessage)
		{
		...
		static string GetPassword(string strMessage)
		{
			Console.WriteLine(strMessage);
		...
		static void Main(string[] args)
        {
			...
			{
				ctx.Credentials = new SharePointOnlineCredentials(strAccountId, oAccountPassword);
				...
				try
				{
					...
					foreach (var library in Libraries)
					{
						...
						do
						{
							...
							Console.WriteLine($"{web.Title} {library.Title} - ({5000 * iCnt}/{iFileCount}) {items.Count()}/{data.Count()}");
					...
					Console.WriteLine($"[{stopwatch.Elapsed}] {web.Title} 사이트 확인 완료");
				}
				catch (Exception ex)
				{
